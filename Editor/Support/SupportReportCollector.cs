...
{
    internal static class SupportReportCollector
    {
        ...
        internal static string Collect(LatticeDeformer deformer)
        {
            var report = new StringBuilder(8192);
            report.AppendLine("Mesh Deformer Support Report");
            Append(report, "format-version", SupportReportCodec.FormatVersion);
        ...
        private static void AppendComponent(StringBuilder report, LatticeDeformer deformer)
        {
            ...
            var output = deformer.GetMeshOutputOptions();
            Append(report, "recalculate-normals", output.RecalculateNormals);
            Append(report, "recalculate-tangents", output.RecalculateTangents);
            Append(report, "recalculate-bounds", output.RecalculateBounds);
        ...
        private static void AppendEditorState(StringBuilder report, LatticeDeformer deformer)
        {
            ...
            var raw = SerializedDeformerReader.Read(deformer);
            var groups = raw.Groups;
            int groupIndex = raw.EmbeddedActiveGroupIndex;
            var group = groups != null && groupIndex >= 0 && groupIndex < groups.Count ? groups[groupIndex] : null;
            var layers = group?.SerializedLayers;
            int layerIndex = group?.SerializedActiveLayerIndex ?? -1;
            var layer = layers != null && layerIndex >= 0 && layerIndex < layers.Count ? layers[layerIndex] : null;
            if (layer != null)
        ...
        {
            bool available = SupportPreviewInspection.TryReadFilters(out var descriptions);
            Append(report, "filter-order-available", available);
            Append(report, "filter-count", descriptions.Count);
            for (int i = 0; i < descriptions.Count; i++) Append(report, $"filter[{i}]", descriptions[i]);
        }
