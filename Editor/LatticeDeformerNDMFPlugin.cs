...
namespace Net._32Ba.LatticeDeformationTool.Editor
{
    internal sealed class LatticeDeformerNDMFPlugin : Plugin<LatticeDeformerNDMFPlugin>
    {
        ...
        protected override void Configure()
        {
            ...
            InPhase(BuildPhase.Transforming)
                .Run(BrushDeformerBakePass.Instance)
                .PreviewingWith(new BrushDeformerPreviewFilter())
                .BeforePlugin("com.anatawa12.avatar-optimizer");
        }
    }
    ...
    internal sealed class BrushDeformerBakePass : Pass<BrushDeformerBakePass>
    {
        protected override void Execute(BuildContext context)
        {
            var deformers = context.AvatarRootObject.GetComponentsInChildren<BrushDeformer>(true);
            if (deformers == null || deformers.Length == 0) return;
        ...
        private static void ProcessDeformer(BuildContext context, BrushDeformer deformer)
        {
            ...
            if (sourceMesh == null)
            {
                if (skinnedMesh != null) sourceMesh = skinnedMesh.sharedMesh;
            ...
            var exportMesh = Object.Instantiate(bakedMesh);
            if (!string.IsNullOrEmpty(ownerName))
                exportMesh.name = ownerName + "_BrushBaked";

            // Recalculate bone weights if enabled
            if (deformer.RecalculateBoneWeights && skinnedMesh != null && sourceMesh.boneWeights != null && sourceMesh.boneWeights.Length > 0)
            {
                var settingsData = deformer.WeightTransferSettings;
                var settings = new WeightTransferSettings
                ...
                {
                    exportMesh.boneWeights = result.weights;
                    Debug.Log($"[BrushDeformer] Weight transfer completed for {ownerName}: {result.transferredCount} transferred, {result.inpaintedCount} inpainted out of {result.totalVertices} vertices.");
                }
                else
                {
                    Debug.LogWarning($"[BrushDeformer] Weight transfer failed for {ownerName}: {result.errorMessage}");
                }
            }
            ...
            if (skinnedMesh != null) skinnedMesh.sharedMesh = exportMesh;
            else if (meshFilter != null) meshFilter.sharedMesh = exportMesh;
