...
- 旧バージョンの `LatticeDeformer`（単一 `_settings` で制御点を保持）を読み込んだ際、  
  `_layers` が空かつ `_settings` にカスタム制御点がある場合は自動でレイヤースタックへ移行する。
- 移行時は旧 `_settings` を `Lattice Layer`（weight=1）として `_layers[0]` にコピーし、  
  ベース `_settings` は中立（neutral）制御点に戻して構造共有用として扱う。
- これにより見た目の変形結果は維持したまま、以降はレイヤー前提の編集フローに移行できる。
...
- `Tests/Editor/LatticeDeformerLayerStackTests.cs` にレイヤースタックの回帰テストを追加済み
  - `AddLayer_CreatesNeutralLayerWithoutChangingBase`
  - `LayerWeight_OffsetsVerticesFromNeutralDelta`
  - `SyncLayerStructuresToBase_ResetsIncompatibleLayerShape`
